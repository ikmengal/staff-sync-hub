...
class EstimateResource extends JsonResource
{
    ...
    public function toArray(Request $request): array
    {
        return [
            ...
            'request' => isset($this->requestData) && !empty($this->requestData) ? $this->requestData->subject : null,
            'title' => $this->title ?? null,
            'price' => $this->price ?? null,
            'status' => isset($this->status) ? ($this->status == 1 ? 'Pending' : ($this->status == 2 ? 'Approved' : ($this->status == 3 ? 'Rejected' : null))) : null,
            'images' => isset($this->attachments) && !empty($this->attachments) ? AttachmentResource::collection($this->attachments) : null,
        ];
    }
}

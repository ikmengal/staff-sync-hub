...
(function() {
    ...
    // If the current time is after midnight (i.e., after 12:00 AM), consider the previous day as active
    if (currentHour < 12) {
        const previousDate = new Date(currentDate); // Create a copy of the current date
        previousDate.setDate(previousDate.getDate() - 1); // Set the date to the previous day
        currentDayIndex = previousDate.getDay(); // Get the index of the previous day
    } else {
        currentDayIndex = currentDate.getDay(); // Get the index of the current day
    }

    // Create a new colors array with the same colors as before
    const colors = [
        config.colors_label.primary,
        config.colors_label.primary,
        config.colors_label.primary,
        config.colors_label.primary,
        config.colors_label.primary, // Placeholder color for the current day
    ];

    // Set the color for the current day dynamically
    // Set the color for the current day
    if (currentDayIndex >= 1 && currentDayIndex <= 5) { // Check if the current day is Monday to Friday
        colors[currentDayIndex - 1] = config.colors.primary; // Subtract 1 from the index to match the array index
    }

...
<div class="row">
    <div class="mb-3 col-4">
        ...
        <select name="currency" id="currency" class="form-control">
            ...
            <option value="{{$currency->code ?? ''}}" @if(isset($data['model']->salaryHistory) && !empty($data['model']->salaryHistory) && $data['model']->salaryHistory->currency_code == $currency->code) selected @endif>{{$currency->title ?? '-'}}</option>
            @endforeach
            @endif
        </select>
        <input type="hidden" name="conversion_amount_hidden" class="conversion_amount_hidden" value="{{$data['model']->salaryHistory->conversion_amount ?? ''}}">
        <input type="hidden" name="conversion_rate" class="conversion_rate" value="{{$data['model']->salaryHistory->currency_rate ?? ''}}">
        <div class="fv-plugins-message-container invalid-feedback"></div>
        <span id="currency_error" class="text-danger error"></span>
        <span class="currency_rate_after_conversion" style="font-size: 13px;opacity: .7;"> @if(!empty($data['model']->salaryHistory->currency_code) && $data['model']->salaryHistory->currency_code != "PKR" ) After Conversion : {{!empty($data['model']->salaryHistory->currency_code)  ? number_format($data['model']->salaryHistory->conversion_amount, 2) : ''}} PKR @endif</span>
    </div>
    ...
    <div class="mb-3 col-4">
        ...
        <input type="date" class="form-control flatpickr-validation" @if(isset($data['model']->jobHistory) && !empty($data['model']->jobHistory->joining_date)) value="{{ $data['model']->jobHistory->joining_date }}" @endif id="joining_date" name="joining_date" required />
        <div class="fv-plugins-message-container invalid-feedback"></div>
